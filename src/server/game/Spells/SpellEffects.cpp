...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
    ...
    switch (properties->Control)
    {
        ...
        case SUMMON_CATEGORY_UNK:
            ...
            switch (SummonTitle(properties->Title))
            {
                ...
                case SummonTitle::Mount:
                    summon = m_caster->GetMap()->SummonCreature(entry, *destTarget, properties, duration, m_originalCaster, m_spellInfo->Id, 0, privateObjectOwner, this);
                    break;
        ...
        case SUMMON_CATEGORY_VEHICLE:
            ...
            summon = m_originalCaster->GetMap()->SummonCreature(entry, *destTarget, properties, duration, m_caster, m_spellInfo->Id, 0, privateObjectOwner, this);
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties, uint32 numGuardians, ObjectGuid privateObjectOwner)
{
    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
        ...
        TempSummon* summon = map->SummonCreature(entry, pos, properties, duration, caster, m_spellInfo->Id, 0, privateObjectOwner, this);
        if (!summon)
            return;
